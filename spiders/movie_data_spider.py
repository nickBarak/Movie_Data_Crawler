...
class MovieDataSpider(Spider):
    ...
    def parse(self, response):
        ...
        next_set = response.urljoin(response.css('div.nav div.desc a.next-page::attr(href)').get())
        yield Request(next_set, callback=self.parse_in_detail, meta={'url': next_set})

    def parse_in_detail(self, response):
        linkgen = self.linkgen(response)
        for link in range(50):
            next_title = linkgen.__next__()
            yield response.follow(next_title, callback=self.store_record)
        ...
        if (next_set is not None):
            yield Request(next_set, callback=self.parse_in_detail)
